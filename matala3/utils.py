...
def compute_mse(image11, image22, border_size=0):
    image1 = np.array(image11)
    image2 = np.array(image22)
        ...
        for x in range(width):
            for y2 in range(scale):
                for x2 in range(scale):
                    multi_channel_image[y, x, y2 * scale + x2] = np_image[y * scale + y2, x * scale + x2, 0]
